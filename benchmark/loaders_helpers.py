...
def calculate_psnr(img1, img2, max_value=1):
    ...
    if len(img2.shape) == 3:
        mse = np.mean((np.array(img1, dtype=np.float32) - np.array(img2[:, :, :, None],
                                                                   dtype=np.float32)) ** 2)
    else:
    ...
    if len(img1.shape) == 3:
        ...
        for kk in range(ssims.shape[0]):
            if len(img2.shape) == 4:
                ...
                ssims[kk] = metrics.structural_similarity(img1[:, :, :, kk], img2,
                                                          data_range=img2.max() - img2.min(),
                                                          channel_axis=2)
...
def calc_tiou(gt_traj, traj, rad):
    ...
    if traj.shape[0] == 4:
        ...
        if bline:
            ...
            for ni in range(ns):
                est_traj[:, ni] = traj[[4, 0]] + np.min([piece * ni, len1]) * v1 + np.max(
                    [0, piece * ni - len1]) * v2
        else:
            for ni, ti in zip(range(ns), np.linspace(0, 1, ns)):
...
def rev_crop_resize(inp, bbox, I):
    ...
    for hsk in range(inp.shape[3]):
        est_hs[bbox[0]:bbox[2], bbox[1]:bbox[3], :, hsk] = \
            cv2.resize(inp[:, :, :, hsk], (bbox[3] - bbox[1], bbox[2] - bbox[0]),
...
def rgba2hs(rgba, bgr):
    ab_mean = rgba[:, :, :1].mean(dim=-1, keepdim=True)
    ab_mean = ab_mean.clip(0, 1)
    ab_mean[ab_mean > 0.1] = 1.0
    return bgr[:, :, :, None] - (ab_mean * rgba[:, :, 1:])
