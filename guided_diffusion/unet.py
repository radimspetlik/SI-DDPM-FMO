...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, record_latents=False):
        latents = []
        for layer in self:
...
class UNetModel(nn.Module):
    ...
    def __init__(self, image_size, in_channels, model_channels, out_channels, num_res_blocks,
                 ...
                 temporal_super_resolution_steps=0):
        super().__init__()
    ...
    def forward(self, x, timesteps, y=None, gt=None, **kwargs):
        ...
        for module in self.input_blocks:
            h = module(h, emb)
            hs.append(h)
        h, latents = self.middle_block(h, emb, True)
        for module in self.output_blocks:
            h = th.cat([h, hs.pop()], dim=1)
            h = module(h, emb)
        h = h.type(x.dtype)

        if self.conf.is_tst is not None and self.conf.is_tst:
            return self.out(h)
...
class SuperResModel(UNetModel):
    ...
    def __init__(self, image_size, in_channels, *args, **kwargs):
        super().__init__(image_size, in_channels * 2, **kwargs)
