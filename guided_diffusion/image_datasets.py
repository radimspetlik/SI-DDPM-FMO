...
from guided_diffusion.blender_generated import BlenderShapeNetGeneratedDataset, \
    BlenderGeneratedDatasetHdf5
...
def load_data(
    ...
    dataset_type = BlenderShapeNetGeneratedDataset if 'data_dir' in ds_conf['paths']['default'] \
        else BlenderGeneratedDatasetHdf5

    dataset = dataset_type(conf,
                           ds_conf,
                           world_size=dist_util.world_size(),
                           global_rank=dist_util.global_rank(),
                           is_trn=dset == 'train')

    if deterministic:
        loader = DataLoader(
            dataset, batch_size=conf.batch_size, shuffle=False, num_workers=1, drop_last=True
        )
    else:
        loader = DataLoader(
            dataset, batch_size=conf.batch_size, shuffle=True, num_workers=0, drop_last=True
        )
